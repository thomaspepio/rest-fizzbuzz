...
func Test_FizzBuzzEndpoint_ShouldCorrectlyCountURLs(t *testing.T) {
	...
	assert.Equal(t, "{\"request\":{\"limit\":15,\"fizzer\":3,\"buzzer\":5,\"fizz\":\"fizz\",\"buzz\":\"buzz\"},\"count\":2}", w.Body.String(), "Top request should be 15-3-5-fizz-buzz")
}
...
func Test_FizzBuzzEndpoint_BadRequest_WhenLimitParameterIsAbsent(t *testing.T) {
	...
	assert.Equal(t, "{\"errorMessage\":\"at least one mandatory parameter is absent\"}", w.Body.String())
...
func Test_FizzBuzzEndpoint_BadRequest_WhenLimitIsNotANumber(t *testing.T) {
	...
	assert.Equal(t, 400, w.Code)
	assert.Equal(t, "{\"errorMessage\":\"could not convert parameter limit to a number\"}", w.Body.String())
	assert.Equal(t, 0, len(urlCounter), fmt.Sprintf("%s should not have been visited", url))
}
...
func Test_FizzBuzzEndpoint_BadRequest_WhenFizzerIsNotANumber(t *testing.T) {
	...
	assert.Equal(t, 400, w.Code)
	assert.Equal(t, "{\"errorMessage\":\"could not convert parameter fizzer to a number\"}", w.Body.String())
	assert.Equal(t, 0, len(urlCounter), fmt.Sprintf("%s should not have been visited", url))
}
...
func Test_FizzBuzzEndpoint_BadRequest_WhenBuzzerIsNotANumber(t *testing.T) {
	...
	assert.Equal(t, 400, w.Code)
	assert.Equal(t, "{\"errorMessage\":\"could not convert parameter buzzer to a number\"}", w.Body.String())
	assert.Equal(t, 0, len(urlCounter), fmt.Sprintf("%s should not have been visited", url))
}
